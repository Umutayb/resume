...
Programming is my passion — I build reusable modules and publish them as open source. The centrepiece is **Civitas Cerebrum** (npm, `@civitas-cerebrum/*`): the AI-native test automation ecosystem beneath ***Magnum Opus***, my autonomous agentic QA platform — a single project-level configuration turns any repository into an autonomous QA target. The libraries separate selectors from test logic, unify execution across web / mobile / desktop, and give LLM assistants the structured primitives they need to author reliable tests. Selected projects:
